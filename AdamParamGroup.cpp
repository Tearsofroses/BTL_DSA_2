...
void AdamParamGroup::zero_grad(){
    //YOUR CODE IS HERE
    for (int i = 0; i < m_pGrads->size(); i++){
        double_tensor* ptr_grad = m_pGrads->get(to_string(i));
        *ptr_grad = xt::zeros<double>(ptr_grad->shape());
    }
}

void AdamParamGroup::step(double lr){
    //YOUR CODE IS HERE
    const double epsilion = 1e-8;
    DLinkedList<string> keys = m_pGrads->keys();
    for (auto key : keys) {
        xt::xarray<double>& P = *m_pParams->get(key);
        xt::xarray<double>& grad_P = *m_pGrads->get(key);
        xt::xarray<double>& mt = *m_pFirstMomment->get(key);
        xt::xarray<double>& vt = *m_pSecondMomment->get(key);
        mt = m_beta1 * mt + (1 - m_beta1) * grad_P;
        vt = m_beta2 * vt + (1 - m_beta2) * grad_P * grad_P;
        xt::xarray<double> mt_hat = mt / (1 - m_beta1_t);
        xt::xarray<double> vt_hat = vt / (1 - m_beta2_t);
        P = P - lr * mt_hat / (xt::sqrt(vt_hat) + epsilion);
    }
    // w_t1 = w_t - alpha * mt
