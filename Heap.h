...
void Heap<T>::remove(T item, void (*removeItemData)(T)) {
  ...
  if (idx == -1) return;
  if (removeItemData) removeItemData(elements[idx]);
...
void Heap<T>::reheapUp(int position) {
  ...
  if (aLTb(elements[position], elements[parent_idx])) {
    swap(position, parent_idx);
    reheapUp(parent_idx);
  }
}
...
void Heap<T>::reheapDown(int position) {
  ...
  int left_child_idx = 2 * position + 1;
  if (left_child_idx >= count) return;
  int right_child_idx = left_child_idx + 1;
  int smaller_child_idx = left_child_idx;
  if (right_child_idx < count && aLTb(elements[right_child_idx], elements[left_child_idx])) {
    smaller_child_idx = right_child_idx;
  }
  if (aLTb(elements[smaller_child_idx], elements[position])) {
    swap(position, smaller_child_idx);
    reheapDown(smaller_child_idx);
  }
}
